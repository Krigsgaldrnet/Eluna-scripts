...
-- Frame pool: recycle hidden toast frames instead of creating new ones
local toastFramePool = {}
local MAX_TOAST_POOL_SIZE = 10
...
-- Remove a toast from the stack and return frame to pool
function ToastManagerMethods:RemoveToast(frame)
    for i, toastData in ipairs(self.activeToasts) do
        if toastData.frame == frame then
            table.remove(self.activeToasts, i)
            -- Return frame to pool for reuse (cap pool size)
            frame:SetScript("OnUpdate", nil)
            frame:Hide()
            frame:ClearAllPoints()
            frame:SetAlpha(0)
            frame.fadeElapsed = nil
            frame.moveElapsed = nil
            frame.startY = nil
            frame.targetY = nil
            if #toastFramePool < MAX_TOAST_POOL_SIZE then
...
function CreateStyledToast(text, duration, fadeTime, anchor)
    ...
    local toast = table.remove(toastFramePool)
    if toast then
        toast:ClearAllPoints()
        toast:SetBackdropColor(UISTYLE_COLORS.ButtonBg[1], UISTYLE_COLORS.ButtonBg[2], UISTYLE_COLORS.ButtonBg[3], 1)
        toast:SetBackdropBorderColor(UISTYLE_COLORS.ButtonBorder[1], UISTYLE_COLORS.ButtonBorder[2], UISTYLE_COLORS.ButtonBorder[3], 1)
        -- Reuse existing FontString
        local message = toast.messageText
...
function ClearAllToasts()
    -- Clear toasts from all managers and return frames to pool (capped)
    for anchor, manager in pairs(ToastManagers) do
        for i = #manager.activeToasts, 1, -1 do
            ...
            toast:ClearAllPoints()
            toast:SetAlpha(0)
            toast.fadeElapsed = nil
            toast.moveElapsed = nil
            toast.startY = nil
            toast.targetY = nil
            if #toastFramePool < MAX_TOAST_POOL_SIZE then
                table.insert(toastFramePool, toast)
            end
        end
