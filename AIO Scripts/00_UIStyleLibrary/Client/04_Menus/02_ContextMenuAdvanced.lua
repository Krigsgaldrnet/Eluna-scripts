...
-- Show or create a rich tooltip on a menu item with tooltipTitle/tooltipText fields
local function ShowMenuItemRichTooltip(menuItem)
    if not menuItem.tooltipTitle and not menuItem.tooltipText then return end
    if not menuItem.richTooltip then
        menuItem.richTooltip = CreateStyledTooltipFrame(menuItem, "TOPRIGHT", 5, 0, 280)
    end
    local content = {}
    if menuItem.tooltipTitle then
        table.insert(content, { text = menuItem.tooltipTitle, size = "large", color = { 1, 1, 1 } })
    end
    if menuItem.tooltipText then
        local processed = menuItem.tooltipText:gsub("\\n", "\n")
        for line in string.gmatch(processed .. "\n", "([^\n]*)\n") do
            if line and line ~= "" then
                table.insert(content, { text = line, color = { 0.8, 0.8, 0.8 } })
            end
        end
    end
    if #content > 0 then
        menuItem.richTooltip:SetContent(content)
        menuItem.richTooltip:Show()
    end
end
