...
-- ===================================
-- UI STYLE LIBRARY EDIT BOX & SLIDERS MODULE
-- ===================================
-- Edit boxes, sliders, and slider with range components
...
function CreateStyledEditBox(parent, width, numeric, maxLetters, multiLine)
    local container = CreateStyledFrame(parent, UISTYLE_COLORS.ButtonBg)
    ...
    if multiLine then
        ...
        -- Style the scroll bar (guard nil name for unnamed frames)
    ...
    editBox:SetScript("OnEditFocusLost", function(self)
        container:SetBackdropBorderColor(UISTYLE_COLORS.ButtonBorder[1], UISTYLE_COLORS.ButtonBorder[2], UISTYLE_COLORS.ButtonBorder[3], 1)
    end)
...
function CreateStyledSlider(parent, width, height, min, max, step, defaultValue, orientation)
    ...
    slider:SetBackdropColor(UISTYLE_COLORS.ButtonBg[1], UISTYLE_COLORS.ButtonBg[2], UISTYLE_COLORS.ButtonBg[3], 1)
...
function CreateStyledSliderWithRange(parent, width, height, min, max, step, defaultValue, labelText)
    ...
    -- Label (if provided and not empty)
    local label
...
--[[
Simulates Enable/Disable for EditBox frames (WoW 3.3.5 EditBoxes lack these methods).
Blocks mouse/keyboard input and greys out text when disabled.
@param editBox - The EditBox frame
@param enabled - true to enable, false to disable
]]
function SetEditBoxEnabled(editBox, enabled)
    ...
    if enabled then
        ...
        editBox:ClearFocus()
        editBox:SetTextColor(0.4, 0.4, 0.4)
    end
end

-- Register this module
UISTYLE_LIBRARY_MODULES = UISTYLE_LIBRARY_MODULES or {}
UISTYLE_LIBRARY_MODULES["EditBoxSliders"] = true

if UISTYLE_DEBUG then
    print("UIStyleLibrary: EditBoxSliders module loaded")
end
