...
function CreateStyledScrollBar(parent, width, height, orientation)
    ...
    -- Track (background) - use button styling for visibility
    local track = scrollBar:CreateTexture(nil, "BACKGROUND")
    track:SetTexture("Interface\\Buttons\\WHITE8X8")
    track:SetVertexColor(UISTYLE_COLORS.ButtonBg[1], UISTYLE_COLORS.ButtonBg[2], UISTYLE_COLORS.ButtonBg[3], 1)
    track:SetAllPoints()

    -- Border - use button border styling for visibility
    scrollBar:SetBackdrop({
        edgeFile = "Interface\\Buttons\\WHITE8X8",
        edgeSize = 1,
    })
    scrollBar:SetBackdropBorderColor(UISTYLE_COLORS.ButtonBorder[1], UISTYLE_COLORS.ButtonBorder[2], UISTYLE_COLORS.ButtonBorder[3], 1)

    -- Thumb (the draggable part) - visible against button background
    local thumb = scrollBar:CreateTexture(nil, "OVERLAY")
    thumb:SetTexture("Interface\\Buttons\\WHITE8X8")
    thumb:SetVertexColor(UISTYLE_COLORS.ButtonBorder[1] - 0.05, UISTYLE_COLORS.ButtonBorder[2] - 0.05, UISTYLE_COLORS.ButtonBorder[3] - 0.05, 1) -- Slightly darker than border
    ...
    scrollBar:SetScript("OnEnter", function(self)
        self.isHovered = true
        thumb:SetVertexColor(UISTYLE_COLORS.ButtonBorder[1], UISTYLE_COLORS.ButtonBorder[2], UISTYLE_COLORS.ButtonBorder[3], 1) -- Match border on hover
    end)

    scrollBar:SetScript("OnLeave", function(self)
        self.isHovered = false
        if not self.isDragging then
            thumb:SetVertexColor(UISTYLE_COLORS.ButtonBorder[1] - 0.05, UISTYLE_COLORS.ButtonBorder[2] - 0.05, UISTYLE_COLORS.ButtonBorder[3] - 0.05, 1)
        end
    end)
    ...
    scrollBar:SetScript("OnMouseDown", function(self)
        ...
        thumb:SetVertexColor(UISTYLE_COLORS.ButtonBorder[1] + 0.05, UISTYLE_COLORS.ButtonBorder[2] + 0.05, UISTYLE_COLORS.ButtonBorder[3] + 0.05, 1) -- Lighter when dragging
    end)

    scrollBar:SetScript("OnMouseUp", function(self)
        self.isDragging = false
        if self.isHovered then
            thumb:SetVertexColor(UISTYLE_COLORS.ButtonBorder[1], UISTYLE_COLORS.ButtonBorder[2], UISTYLE_COLORS.ButtonBorder[3], 1)
        else
            thumb:SetVertexColor(UISTYLE_COLORS.ButtonBorder[1] - 0.05, UISTYLE_COLORS.ButtonBorder[2] - 0.05, UISTYLE_COLORS.ButtonBorder[3] - 0.05, 1)
        end
    end)
