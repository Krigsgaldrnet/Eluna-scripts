...
-- Generate cache key for item search results
local function generateItemSearchCacheKey(searchText, category, quality, slotId, minLevel, maxLevel, minItemLevel, maxItemLevel, stackableOnly, sortBy, sortOrder)
    ...
    local params = {
        ...
        minItemLevel or 0,
        maxItemLevel or 999,
        stackableOnly and "true" or "false",
        sortBy or "ItemLevel",
        sortOrder or "DESC"
    }

    return "itemsearch:" .. table.concat(params, ":")
end
...
function ItemSearchHandlers.requestModalItems(player, searchText, category, qualitiesStr)
    ...
    DatabaseHelper.SafeQueryAsync(query, function(results, error)
        local items = {}
        ...
        elseif Config.debug then
            print(string.format("[GameMasterUI] Failed to get modal items: %s", error or "unknown error"))
        end
        ...
        --[[
        print(string.format("[ItemSearchHandlers] About to send packets for %d modal items", #items))
        if _G.GameMasterUI_SendItemQueryForList and _G.GameMasterUI_ExtractItemEntries then
            print("[ItemSearchHandlers] Global functions found, calling...")
            local itemEntries = _G.GameMasterUI_ExtractItemEntries(items)
            _G.GameMasterUI_SendItemQueryForList(player, itemEntries)
        else
            print("[ItemSearchHandlers] ERROR: Global functions not found!")
            print("SendItemQueryForList:", _G.GameMasterUI_SendItemQueryForList)
            print("ExtractItemEntries:", _G.GameMasterUI_ExtractItemEntries)
        end
        --]]
...
function ItemSearchHandlers.searchItems(player, searchText, category, quality, slotId, 
    ...
    local cacheKey = generateItemSearchCacheKey(searchText, category, quality, slotId,
                                               minLevel, maxLevel, minItemLevel, maxItemLevel,
                                               stackableOnly, sortBy, sortOrder)

    -- Check cache first
    local cachedResults = QueryCache.get("itemData", cacheKey)
    if cachedResults then
        debugMessage(string.format("[searchItems] Cache hit for key: %s", cacheKey))
        AIO.Handle(player, "GameMasterSystem", "receiveItemSearchResults", cachedResults)
        return
    end
    ...
    debugMessage("[searchItems] Cache miss, executing query:", query)
    ...
    DatabaseHelper.SafeQueryAsync(query, function(result, error)
        ...
        if result then
            ...
            -- Cache the results for future requests (TTL: 10 minutes for item searches)
            QueryCache.set("itemData", cacheKey, items, 600)
            debugMessage(string.format("[searchItems] Cached %d items with key: %s", #items, cacheKey))
        elseif Config.debug then
            print(string.format("[GameMasterUI] Failed to search items: %s", error or "unknown error"))
        end
        ...
        -- To re-enable, set ENABLE_ITEM_PACKETS = true in GameMasterUI_Init.lua
        --[[
        print(string.format("[ItemSearchHandlers] About to send packets for %d search items", #items))
        if _G.GameMasterUI_SendItemQueryForList and _G.GameMasterUI_ExtractItemEntries then
            print("[ItemSearchHandlers] Global functions found for search, calling...")
            local itemEntries = _G.GameMasterUI_ExtractItemEntries(items)
            _G.GameMasterUI_SendItemQueryForList(player, itemEntries)
        else
            print("[ItemSearchHandlers] ERROR: Global functions not found for search!")
            print("SendItemQueryForList:", _G.GameMasterUI_SendItemQueryForList)
            print("ExtractItemEntries:", _G.GameMasterUI_ExtractItemEntries)
        end
        --]]
...
function ItemSearchHandlers.addItemToSpecificSlot(player, targetName, itemId, quantity, bagId, slotId)
    ...
    -- Verify item exists asynchronously
    local itemQuery = "SELECT name FROM item_template WHERE entry = " .. itemId
    DatabaseHelper.SafeQueryAsync(itemQuery, function(result, error)
        if not result then
            Utils.sendMessage(player, "error", "Item ID " .. itemId .. " does not exist.")
            return
        end

        local itemName = result:GetString(0)
...
function ItemSearchHandlers.equipItemById(player, targetName, itemId, slotId)
    ...
    -- Verify item exists and can be equipped asynchronously
    local itemQuery = string.format("SELECT name, InventoryType FROM item_template WHERE entry = %d", itemId)
    DatabaseHelper.SafeQueryAsync(itemQuery, function(result, error)
        if not result then
            Utils.sendMessage(player, "error", "Item ID " .. itemId .. " does not exist.")
            return
        ...
        local itemName = result:GetString(0)
        local inventoryType = result:GetUInt32(1)
        ...
        if PlayerHandlers and PlayerHandlers.getPlayerInventory then
            -- Sending fresh inventory data after equip
