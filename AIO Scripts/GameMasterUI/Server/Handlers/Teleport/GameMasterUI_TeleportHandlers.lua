...
local GameMasterSystem, Config, Utils, DatabaseHelper, DatabaseErrorHelper
...
function TeleportHandlers.GetTeleportLocations(player, page, pageSize, sortOrder)
    ...
    -- Get total count using DatabaseErrorHelper
    local countQuery = "SELECT COUNT(*) as total FROM game_tele"
    local countResult, countError = DatabaseErrorHelper.SafeQueryWithClientError(
        player,
        countQuery,
        {"game_tele"},
        "world",
        "Loading teleport locations count"
    )
    ...
    elseif countError then
        -- Error already sent to client by DatabaseErrorHelper
        return
    end
    ...
    local result, queryError = DatabaseErrorHelper.SafeQueryWithClientError(
        player,
        query,
        {"game_tele"},
        "world",
        "Loading teleport locations"
    )
...
function TeleportHandlers.RegisterHandlers(gms, config, utils, database, dbHelper, dbErrorHelper)
