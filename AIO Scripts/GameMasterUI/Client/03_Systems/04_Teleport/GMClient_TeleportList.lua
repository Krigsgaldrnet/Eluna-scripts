...
function Teleport.ShowTeleportList(targetPlayer)
    ...
    teleportModal.refreshBtn = refreshBtn  -- Store for hotkey access
    ...
    teleportModal:SetScript("OnHide", function(self)
...
-- Close teleport dialog (for state machine integration)
function Teleport.CloseTeleportList()
    if teleportModal and teleportModal:IsShown() then
        teleportModal:Hide()
    end
end
...
GameMasterSystem.CloseTeleportList = Teleport.CloseTeleportList
