...
-- Template Editor confirmation dialogs
StaticPopupDialogs["ITEMTEMPLATEEDITOR_CONFIRM_DELETE"] = {
    text = "Delete item template '%s' (ID: %s)?\n\n|cffff0000This action cannot be undone!|r",
    button1 = "Delete",
    button2 = "Cancel",
    timeout = 0,
    whileDead = true,
    ...
    preferredIndex = 3,
    OnAccept = function(self, data)
        if _G.ItemTemplateEditor and _G.ItemTemplateEditor.DeleteTemplate then
            _G.ItemTemplateEditor.DeleteTemplate()
        end
    end,
}

-- Database Error Dialog
StaticPopupDialogs["GM_DATABASE_ERROR"] = {
    text = "Database Error\n\n%s\n\n|cFFFFFF00Missing Table(s):|r\n%s",
    button1 = "Close",
    button2 = "Retry",
    timeout = 0,
    whileDead = true,
    ...
    OnCancel = function(self, data)
        -- Retry - refresh current view
        if data and data.retryCallback then
            data.retryCallback()
        elseif GMUI and GMUI.RefreshCurrentView then
            GMUI.RefreshCurrentView()
        end
    end,
}

-- Database Warning Dialog (for optional tables)
StaticPopupDialogs["GM_DATABASE_WARNING"] = {
    text = "Database Warning\n\n%s\n\n|cFFFFFF00Some features may not be available.|r",
    button1 = "OK",
    timeout = 0,
    whileDead = true,
    hideOnEscape = true,
