...
function GameMasterSystem.ShowGiveGoldDialog(playerName)
    -- Check if we can open a dialog through state machine
    if StateMachine and not StateMachine.canOpenModal() then
        if _G.GM_DEBUG then
            print("[DialogHandlers] Cannot open gold dialog - system busy or modal already open")
        end
        return false
    end

    -- Transition to dialog open state
    if StateMachine then
        StateMachine.openDialog("gold")
    end
