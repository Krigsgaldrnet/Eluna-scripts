...
-- 3.3.5 compatible timer function (C_Timer doesn't exist in 3.3.5)
...
function EntitySelectionDialog.CreateDialog()
    ...
    HotkeyManager.registerModalHotkeys({
        modal = dialog,
        context = "entity_selection_dialog",
        priority = 80,
        onRefresh = function()
        ...
        onFocusSearch = function()
            if searchBox and searchBox.editBox then
                searchBox.editBox:SetFocus()
                searchBox.editBox:HighlightText()
            end
        end
    })

    -- Unregister hotkeys and notify state machine when dialog is hidden
    dialog:SetScript("OnHide", function(self)
        ...
        if HotkeyManager then
            for _, handlerId in ipairs(self.hotkeyHandlerIds or {}) do
                HotkeyManager.unregister(handlerId)
            end
            self.hotkeyHandlerIds = {}
        end
    end)
...
function EntitySelectionDialog.CreateListItem(parent, entity, index)
    ...
    item:RegisterForClicks("LeftButtonUp", "RightButtonUp")  -- Enable both left and right clicks
    ...
    elseif entity.entityType == "GameObject" then
        ...
        GMUtils.debug("WARNING", string.format("[EntitySelectionDialog] Unexpected entityType: %s for entity %s (Entry: %d)", 
...
function EntitySelectionDialog.ShowContextMenu(entity, anchor)
    local menu = {
        ...
        {
            text = "|cFFFF0000Delete from World|r",  -- Red color for danger
    ...
    -- Use the styled menu system if available, otherwise fall back to regular EasyMenu
...
function EntitySelectionDialog.EditEntity(entity)
    GMUtils.debug("INFO", string.format("[EntitySelectionDialog] EditEntity called for %s: %s (Entry: %d, GUID: %s, Type: %s)", 
    ...
    if entity.entityType == "Creature" then
        GMUtils.debug("INFO", string.format("[EntitySelectionDialog] Sending creature edit request for GUID: %s", tostring(entity.guid)))
        AIO.Handle("GameMasterSystem", "getCreatureForEdit", entity.guid)
    else
        GMUtils.debug("INFO", string.format("[EntitySelectionDialog] Sending GameObject edit request for GUID: %s", tostring(entity.guid)))
...
function EntitySelectionDialog.DeleteEntity(entity)
    -- Create styled confirmation dialog instead of StaticPopup (3.3.5 compatibility)
    local confirmDialog = CreateStyledDialog({
        title = "Confirm Delete",
        message = string.format("Are you sure you want to delete %s from the world?\n\n|cFFFF0000This action cannot be undone!|r", 
            entity.name or "this entity"),
        width = 350,
        buttons = {
            {
            ...
            {
                text = "Cancel",
                callback = function() end  -- Just closes dialog
            }
        }
    })
...
function EntitySelectionDialog.RefreshEntities()
    GMUtils.debug("INFO", string.format("[EntitySelectionDialog] Requesting entities with range: %d", EntitySelectionDialog.CONFIG.REFRESH_RANGE))
...
function EntitySelectionDialog.ReceiveEntities(entities)
    GMUtils.debug("INFO", string.format("[EntitySelectionDialog] Received %d entities from server", entities and #entities or 0))
    ...
    if entities and #entities > 0 then
        for i, entity in ipairs(entities) do
            if i <= 5 then -- Log first 5 entities for debugging
                GMUtils.debug("DEBUG", string.format("[EntitySelectionDialog] Entity %d: %s (Entry: %d, Type: %s)", 
...
function EntitySelectionDialog.Open()
    GMUtils.debug("INFO", "[EntitySelectionDialog] Opening entity selection dialog")
    if not EntitySelectionDialog.dialog then
        GMUtils.debug("INFO", "[EntitySelectionDialog] Creating dialog for first time")
    ...
    if EntitySelectionDialog.dialog then
        ...
        GMUtils.error("[EntitySelectionDialog] ERROR: Dialog not created!")
    end
end
...
-- StaticPopup removed - using CreateStyledDialog for 3.3.5 compatibility
