...
function PlayerInventory.createInventoryModalWithLoading(playerName)
    ...
    -- Allow movement outside screen boundaries - will reset to center on reopen
    ...
    -- Enable mouse and register for drag on title bar
    ...
    -- Close button
    local closeBtn = CreateStyledButton(titleBar, "X", 26, 26)
    ...
    -- Click outside to close (only on background) - DISABLED for better usability
    -- Uncomment the following code to re-enable click-outside-to-close functionality
    --[[
    ...
    -- Unregister hotkeys and notify state machine when modal is hidden
...
function PlayerInventory.receiveInventoryData(player, inventoryData, equipmentData, playerName, bagConfiguration, bankData, playerStats)
    -- Store all item data in client-side cache
    local totalCached = 0
    if _G.GMItemCache then
        -- Cache inventory items
        if inventoryData and type(inventoryData) == "table" then
            totalCached = totalCached + _G.GMItemCache.StoreItems(inventoryData)
        end
        -- Cache equipment items
        if equipmentData and type(equipmentData) == "table" then
            local equipList = {}
            for _, item in pairs(equipmentData) do
                table.insert(equipList, item)
            end
            totalCached = totalCached + _G.GMItemCache.StoreItems(equipList)
        end
        -- Cache bank items
        if bankData and type(bankData) == "table" then
            totalCached = totalCached + _G.GMItemCache.StoreItems(bankData)
        end
        
        if GMConfig.config.debug and totalCached > 0 then
            print(string.format("[PlayerInventory] Cached %d items for inventory display", totalCached))
        end
    end
