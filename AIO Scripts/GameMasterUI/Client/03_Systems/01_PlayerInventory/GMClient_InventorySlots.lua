...
function PlayerInventory.createInventorySlot(parent, slotData, isEquipment)
    ...
    function slot:UpdateSlot(itemData)
        ...
        if itemData and itemData.entry and itemData.entry > 0 then
            -- Get item info using cache-aware utility
            local itemName, itemLink, itemQuality, itemLevel, itemMinLevel, itemType, itemSubType, 
                  itemStackCount, itemEquipLoc, itemTexture = GMUtils.GetItemInfo(itemData.entry)
            
            -- If we didn't get data from cache or GetItemInfo, create fallback
            if not itemName then
                itemName = GMUtils.GetColoredItemName(itemData.entry)
                itemLink = GMUtils.GetItemLink(itemData.entry, itemData.enchantId)
                itemQuality = 1  -- Default to Common
                itemTexture = "Interface\\Icons\\INV_Misc_QuestionMark"
            end
            
            -- Update item link with enchantment if present
            local enchantId = itemData.enchantId or 0
            if enchantId > 0 and itemLink and not itemLink:find(":" .. enchantId .. ":") then
                -- Rebuild link with enchantment using cache-aware function
                itemLink = GMUtils.GetItemLink(itemData.entry, enchantId)
            end
            ...
            if itemTexture then
                ...
                self:SetScript("OnEnter", function(frame)
                    ...
                    if itemLink then
                        -- Use cache-aware tooltip setting
                        local usedCache = GMUtils.SetItemTooltip(GameTooltip, itemData.entry, enchantId)
                        if not usedCache then
                            -- Fallback to hyperlink if cache doesn't have tooltip data
