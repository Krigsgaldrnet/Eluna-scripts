...
function GameMasterSystem.receiveItemData(player, data, offset, pageSize, hasMoreData, inventoryType, totalCount, totalPages, currentPage)
    ...
    -- DISABLED: Request item packets for all items to ensure they display properly
    -- Instead, store items in client-side cache for proper display
    if data and type(data) == "table" and #data > 0 then
        -- Store items in client-side cache
        if _G.GMItemCache and _G.GMItemCache.StoreItems then
            local storedCount = _G.GMItemCache.StoreItems(data)
            if GMConfig.config.debug then
                print(string.format("[GMClient_08a_DataHandlers] Stored %d items in client cache", storedCount))
            end
        else
            -- WARNING: GMItemCache not available
        end
        
        -- AIO.Handle("GameMasterSystem", "requestItemPacketsForList", data)  -- DISABLED
    end
    ...
    -- Sync with global state (use sanitized offset to prevent table comparison errors)
...
function GameMasterSystem.receivePlayerData(player, data, offset, pageSize, hasMoreData, totalCount, totalPages, currentPage)
    ...
    -- IMPORTANT: If we're using the new SearchManager, don't process this
    -- The SearchManager has its own callback (PlayerList.receivePlayerData)
    -- This prevents the race condition where old data overwrites new data
    if _G.GMCards and _G.GMCards.PlayerList then
        -- Check if SearchManager was successfully initialized
        if _G.GMCards.PlayerList._searchManagerInitialized then
            return -- SearchManager is active, ignore this legacy handler
        end
    end
    ...
    if _G.GMCards then
        -- Store raw unfiltered data from server
        _G.GMCards.rawPlayerData = data
        -- Also set currentPlayerData for backward compatibility
        _G.GMCards.currentPlayerData = data

        -- Clear waiting flag if using fallback mode
        if _G.GMCards.PlayerList and not _G.GMCards.PlayerList._searchManagerInitialized then
            _G.GMCards.PlayerList.isWaitingForData = false
        end
    end
    ...
    -- DON'T re-apply filters here - it was causing offline players to disappear
    -- The data from the server is already filtered correctly
    -- ReapplyFilters should only be called by user actions (search, class filter)
...
-- Helper function to check if initialization is complete
local function checkInitializationComplete()
    if StateMachine and GMData and GMData.PlayerGMLevel and GMData.CoreName then
        -- Both GM level and core name are received, complete initialization
        GMData.isGmLevelFetched = true
        GMData.isCoreNameFetched = true

        if _G.GM_DEBUG then
            print("[GameMasterSystem] Initialization complete - GM Level:", GMData.PlayerGMLevel, "Core:", GMData.CoreName)
        end
...
function GameMasterSystem.receiveGmLevel(player, gmLevel)
    ...
    if _G.GM_DEBUG then
        print("[GameMasterSystem] GM Level received:", gmLevel)
    end
...
function GameMasterSystem.receiveCoreName(player, coreName)
    ...
    if _G.GM_DEBUG then
        print("[GameMasterSystem] Core Name received:", coreName)
    end
...
function GameMasterSystem.receiveModalItemData(player, items)
    ...
    if items and type(items) == "table" and #items > 0 then
        if _G.GMItemCache and _G.GMItemCache.StoreItems then
            local storedCount = _G.GMItemCache.StoreItems(items)
            if GMConfig.config.debug then
                print(string.format("[GMClient_08a_DataHandlers] Stored %d modal items in client cache", storedCount))
            end
        end
    end
