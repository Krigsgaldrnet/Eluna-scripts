...
function PlayerSpells.createSpellManagerModal(playerName, playerEntity)
    ...
    learnBtn:SetScript("OnClick", function()
        ...
        if StateMachine and StateMachine.canOpenModal() then
            StateMachine.openSpellSelection(playerName, "learn")
        else
            -- Enhanced debug info to help troubleshoot
            if not _G.GMStateMachine then
                CreateStyledToast("State machine not available", 4, 0.5)
...
function PlayerSpells.showLearnSpellDialog(playerName)
    -- Redirect to the comprehensive spell selection dialog via state machine
    local StateMachine = _G.GMStateMachine
    if StateMachine and StateMachine.canOpenModal() then
        StateMachine.openSpellSelection(playerName, "learn")
    else
        CreateStyledToast("State machine not available", 3, 0.5)
    end
end
